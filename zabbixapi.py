...
def pull_zabbix_events():
    ...
    resp1 = call_zabbix_api(
        credentials.ZABBIXURL, credentials.ZABBIXTOKEN, "problem.get", eventparams1
    )
    resp2 = call_zabbix_api(
        credentials.ZABBIXURL, credentials.ZABBIXTOKEN, "problem.get", eventparams2
    )
    resp3 = call_zabbix_api(
        credentials.ZABBIXURL, credentials.ZABBIXTOKEN, "problem.get", eventparams3
    )
    resp4 = call_zabbix_api(
        credentials.ZABBIXURL, credentials.ZABBIXTOKEN, "problem.get", eventparams4
    )
    ...
    for event in combined_results["result"]:
        ...
        problembody = problembody + (
            ", ".join(tag["value"] for tag in event["tags"]) + "\n"
        )
        ...
        problembody = (
            f"{event['eventid']} | {host_names} | {timestamp}\n"
        ) + problembody
        ...
        c.execute(
            """INSERT INTO zabbixevents (timestamp, subject, issuebody, relatedbody)
                    VALUES (?, ?, ?, ?)""",
...
def get_hosts_for_event(eventid):
    """Gets the host names for a given event ID, because Zabbix doesn't include them in the event data."""
